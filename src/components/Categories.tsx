    ...
    >
      <Link to={`/shop?category=${category.name.toLowerCase()}`}>
        ...
        >
          ...
          {/* Gradient Overlay */}
          <div className={`absolute inset-0 bg-gradient-to-br ${category.gradient} mix-blend-multiply opacity-80`} />
          ...
          >
            ...
            >
              <span className="font-semibold text-lg">Explore Collection</span>
              <motion.div
      ...
      <div className="container mx-auto px-4 md:px-6 relative z-10">
            ...
            {[...marqueeItems, ...marqueeItems, ...marqueeItems].map((item, i) => (
              <span 
                ...
                className={`text-2xl md:text-3xl font-bold ${
                  item === "✦" ? "text-primary" : "text-foreground/20"
                }`}
              >
                {item}
              </span>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6 md:gap-8">
          {categories.map((category, index) => (
            <Card3D key={category.id} category={category} index={index} />
          ))}
        </div>
